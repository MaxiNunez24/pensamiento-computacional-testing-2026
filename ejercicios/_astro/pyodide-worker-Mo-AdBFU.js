...
`;let o=null,s=null;async function d(){const e=await(await import(`${n}pyodide.mjs`)).loadPyodide({indexURL:n});e.runPython(l),o=e.globals.get("_run_user"),s=e.globals.get("_medir"),postMessage({type:"ready"})}const c=d().catch(a=>{postMessage({type:"init-error",error:String(a)})});self.onmessage=async a=>{const{id:e,code:t,tests:u,archivo:p,datos:i,entradas:m,modo:f,escenarios:_,tope:b}=a.data;if(await c,!o||!s)return;let r;try{r=f==="medir"?s(t,i||"",JSON.stringify(_||[]),b||2e5):o(t,u,p||"",i||"",JSON.stringify(m||[]))}catch(g){r=JSON.stringify({ok:!1,out:"",err:String(g)})}postMessage({type:"result",id:e,raw:r})}})();

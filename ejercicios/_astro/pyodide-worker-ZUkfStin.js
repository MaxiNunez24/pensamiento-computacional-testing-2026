(function(){"use strict";const p="https://cdn.jsdelivr.net/pyodide/v0.29.4/full/",_=`
...
`;let n=null,t=null,e=null;async function b(){const a=await(await import(`${p}pyodide.mjs`)).loadPyodide({indexURL:p});e=a,a.runPython(_),n=a.globals.get("_run_user"),t=a.globals.get("_medir"),postMessage({type:"ready"})}const g=b().catch(s=>{postMessage({type:"init-error",error:String(s)})});self.onmessage=async s=>{const{id:a,code:i,tests:m,archivo:v,datos:l,entradas:y,archivos:d,modo:f,escenarios:q,tope:x}=s.data;if(await g,!n||!t)return;const c=Object.keys(d||{});if(e&&c.length){for(const r of c)e.FS.writeFile(r,d[r]);const o=c.map(r=>r.replace(/\.py$/,""));e.runPython(["import sys, importlib","for _m in "+JSON.stringify(o)+":","    sys.modules.pop(_m, None)","importlib.invalidate_caches()"].join(`
`))}if(e&&f!=="medir")try{const o=[l||"",i,m,...Object.values(d||{})].join(`
`);await e.loadPackagesFromImports(o,{messageCallback:()=>{}})}catch{}let u;try{u=f==="medir"?t(i,l||"",JSON.stringify(q||[]),x||2e5):n(i,m,v||"",l||"",JSON.stringify(y||[]))}catch(o){u=JSON.stringify({ok:!1,out:"",err:String(o)})}postMessage({type:"result",id:a,raw:u})}})();

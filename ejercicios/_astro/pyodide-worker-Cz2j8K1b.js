(function(){"use strict";const d="https://cdn.jsdelivr.net/pyodide/v0.29.4/full/",p=`
...
`;let o=null,s=null,r=null;async function m(){const e=await(await import(`${d}pyodide.mjs`)).loadPyodide({indexURL:d});r=e,e.runPython(p),o=e.globals.get("_run_user"),s=e.globals.get("_medir"),postMessage({type:"ready"})}const f=m().catch(a=>{postMessage({type:"init-error",error:String(a)})});self.onmessage=async a=>{const{id:e,code:n,tests:c,archivo:_,datos:t,entradas:b,modo:u,escenarios:g,tope:v}=a.data;if(await f,!o||!s)return;if(r&&u!=="medir")try{const l=[t||"",n,c].join(`
`);await r.loadPackagesFromImports(l,{messageCallback:()=>{}})}catch{}let i;try{i=u==="medir"?s(n,t||"",JSON.stringify(g||[]),v||2e5):o(n,c,_||"",t||"",JSON.stringify(b||[]))}catch(l){i=JSON.stringify({ok:!1,out:"",err:String(l)})}postMessage({type:"result",id:e,raw:i})}})();
